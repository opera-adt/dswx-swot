...
Dynamic Surface Water Extent from Surface Water and Ocean Topography (SWOT) Level 2 Water Mask Raster Image Data Product

# Development setup
### Prerequisite installs
1. Download source code:
```bash
git clone https://github.com/opera-adt/dswx-swot.git
```
2. Install dependencies, either to a new environment:
```bash
mamba env create --name dswx-swot --file dswx-swot/environment.yml
conda activate dswx-swot
```
or install within your existing env with mamba.

3. Install `dswx-swot` via pip in editable mode
```bash
python -m pip install --no-deps -e  dswx-swot
```
