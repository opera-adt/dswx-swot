...
class InputOptions(BaseModel, extra="forbid"):
    ...
    input_file_path: str | Path = Field(
        default_factory=str,
        description="Path to the input SWOT L2 HR.nc files",
    )

    date_fmt: str = Field(
        "%Y%m%d",
        description="Format of dates.",
    )


# DynamicAncillary, StatiAncillary
...
class OutputOptions(BaseModel, extra="forbid"):
    ...
    date_fmt: str = Field(
        "%Y%m%dT%H%M%S",
        description="Output Date Format for OPERA DSWx",
    )
    ...
    product_version: str = Field(
        PRODUCT_VERSION,
        description="OPERA DSWx-SWOT product version",
    )
    ...
    def get_output_filename(self, date: str | datetime, hour: str | int):
        ...
        return f"OPERA_L3_DSWx_{datetime_str}_{self.product_version}.nc"
...
class DSWxWorkflow(YamlModel, extra="forbid"):
    """DSWx-SWOT configuration models."""
